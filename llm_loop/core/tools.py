...
class BuiltinToolProvider(ToolProvider):
    ...
    def get_tools(self) -> List[ToolFunction]:
        ...
        try:
            ...
            for name, method in inspect.getmembers(SecureFileOperations, inspect.isfunction):
    ...
    def _load_tools_from_file(self, file_path: str) -> List[ToolFunction]:
        ...
        try:
            if not file_path.endswith('.py') or not pathlib.Path(file_path).exists():
                click.echo(f"Warning: Tool file not found or invalid: {file_path}", err=True)
                return tools
    ...
    def get_all_tools(self) -> List[ToolFunction]:
        ...
        for provider in self.providers:
            try:
                ...
                click.echo(f"Warning: Error loading tools from provider {provider.__class__.__name__}: {e}", err=True)
